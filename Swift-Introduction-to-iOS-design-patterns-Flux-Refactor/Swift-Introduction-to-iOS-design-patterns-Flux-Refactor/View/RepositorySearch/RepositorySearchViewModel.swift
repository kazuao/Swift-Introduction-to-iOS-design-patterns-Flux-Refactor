...
final class RepositorySearchViewModel {
         ...
         flux: Flux)
    {
        ...
        self.reloadRepositories = repositories.changed.map { _ in }
        
        self.editingLayout = searchStore.isSearchFieldEditing
            .asObservable()
            .flatMap { $0 ? Observable.just(()) : .empty() }
        
        self.nonEditingLayout = searchStore.isSearchFieldEditing
            .asObservable()
    ...
    // MARK: Public
    func _selectedIndexPath(_ indexPath: IndexPath) {
